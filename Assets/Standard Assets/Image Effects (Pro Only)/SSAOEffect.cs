...
public class SSAOEffect : MonoBehaviour
{
	...
	void OnEnable () {
        gameObject.GetComponent<Camera>().depthTextureMode |= DepthTextureMode.DepthNormals; //REFACTOR MC 21-06-2016 - Change: camera.depthTextureMode --> gameObject.GetComponent<Camera>().depthTextureMode
	}
	...
	void OnRenderImage (RenderTexture source, RenderTexture destination)
	{
        ...
        float fovY = gameObject.GetComponent<Camera>().fieldOfView; //REFACTOR MLB 21-06-2016 - Change: camera.fieldOfView; --> gameObject.GetComponent<Camera>().fieldOfView;
        float far = gameObject.GetComponent<Camera>().farClipPlane; //REFACTOR MLB 21-06-2016 - Change: camera.farClipPlane; --> gameObject.GetComponent<Camera>().farClipPlane;
		float y = Mathf.Tan (fovY * Mathf.Deg2Rad * 0.5f) * far;
        float x = y * gameObject.GetComponent<Camera>().aspect; //REFACTOR MLB 21-06-2016 - Change: camera.aspect; --> gameObject.GetComponent<Camera>().aspect;
