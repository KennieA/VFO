...
public class Sit_up_borger_b_from_side : MonoBehaviour 
{	
	private void initializeExercise()
	{
        ...
        if (tg)
        {
            tg.SetToolCorrectness("Psoas-pude", true);
            tg.SetToolCorrectness("Sengekontrol", true);
        }
        Util.ToggleResource<ToolGrid>("ToolGrid");

        HUDTiled hud3 = Util.ToggleResource<HUDTiled>("HUD_Box");
        if (hud3)
        {
            hud3.Buttons[(int)Box.Position.BEHIND].Correct = true;
        }
        Util.ToggleResource<HUDTiled>("HUD_Box");
    ...
    {
        // Exercise States
        States.Instance.InitExerciseState(new string[] { "BedHeightDown" }, States.CheckCon.NotCritical, Text.Instance.GetString("sim_sit_b_state_beddown"), 0.0f);
        States.Instance.InitExerciseState(new string[] { "Talk_0_0" }, States.CheckCon.NotCritical, Text.Instance.GetString("sim_sit_b_state_bend"), 0.0f);
        States.Instance.InitExerciseState(new string[] { "Talk_0_1" }, States.CheckCon.NotCritical, Text.Instance.GetString("sim_sit_b_state_arms"), 0.0f);
        States.Instance.InitExerciseState(new string[] { "HUD_Box(Clone)#1" }, States.CheckCon.VideoError, Text.Instance.GetString("sim_sit_b_state_psoas"), "", 0.0f, 0);
        States.Instance.InitExerciseState(new string[] { "Talk_0_2" }, States.CheckCon.NotCritical, Text.Instance.GetString("sim_sit_b_state_ready"), 0.0f);
        States.Instance.InitExerciseState(new string[] { "BedHeadUp" }, States.CheckCon.ShowVideo, Text.Instance.GetString("sim_sit_b_state_bedhead"), "", 7.0f, 0);

        // Help text
        Help.Instance.AddHelpText(new string[] { "start" }, "sim_sit_b_state_help_psoas");
        Help.Instance.AddHelpText(new string[] { "ToolboxDone" }, "sim_sit_b_help_lowerbed");
        Help.Instance.AddHelpText(new string[] { "BedHeightDown" }, "sim_sit_b_help_bend");
        Help.Instance.AddHelpText(new string[] { "Talk_0_0" }, "sim_sit_b_help_arms");
        Help.Instance.AddHelpText(new string[] { "Talk_0_1" }, "sim_sit_b_state_help_psoas_place");
        Help.Instance.AddHelpText(new string[] { "HUD_Box(Clone)#1" }, "sim_sit_b_help_ready");
        Help.Instance.AddHelpText(new string[] { "Talk_0_2" }, "sim_sit_b_help_bedhead");

        // Dialog
        int pos1 = Talk.Instance.AddTalkDialog("hoover_head", "sim_sit_b_talk_bend");
        Talk.Instance.AddTalkDialog(pos1, "sim_sit_b_talk_arms");
        Talk.Instance.AddTalkDialog(pos1, "sim_sit_b_talk_ready");
        
        // Animations Female Helper
        AnimateFemale.Instance.SetIdle("idle_100f");
        AnimateFemale.Instance.AddAnimation(new string[] { "BedHeadUp" }, "1", "C_assist_legs_263f", 0.0f, false, 10);

        // Animations Borger
        AnimateBorger.Instance.SetIdle("Idle_Side_100f");
        AnimateBorger.Instance.AddAnimation(new string[] { "Talk_0_0" }, "1", "Side_bend_legs_55f", 0.0f, false, 10);
        AnimateBorger.Instance.AddAnimation(new string[] { "Talk_0_1" }, "2", "Side_Place_arms_51f", 0.0f, false, 20);
        AnimateBorger.Instance.AddAnimation(new string[] { "BedHeadUp" }, "3", "B_bed_Lying_to_Sitting_263f", 0.0f, false, 30, -1.0f, 2.0f);
        ...
        AnimateBed3.Instance.AddAnimation(new string[] { "BedHeadUp" }, "head_end_pos_high_35_degree", 1.0f, 3.3f);

        //Animate Box
        AnimateBox.Instance.AddResetState(new string[] { "HUD_Box(Clone)#1" });
    }
    ...
    public void SimCallback(string t)
    {
        ...
        if (t != _currentState && !States.Instance.GetExersiciseValue(t) && !States.Instance.HasFinished())
        {
            ...
            else
            {
                ...
                AnimateBox.Instance.UpdateAnimation(t);
