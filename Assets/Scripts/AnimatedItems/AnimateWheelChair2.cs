...
public class AnimateWheelChair2 : MonoBehaviour {
	...
	public class CAnimate
	{
		...
		{
			s = AnimateWheelChair2.Instance.GetComponent<Animation>()[animName];
		...
		public void StartAnim()
		{
			if(additive && !s.enabled) {
				s.enabled = true;
				AnimateWheelChair2._instance.GetComponent<Animation>().CrossFade(s.name, animFadeTime);
			}
			else {
				AnimateWheelChair2.Instance.GetComponent<Animation>().CrossFade(s.name, animFadeTime);
			}
		}
	...
	private static AnimateWheelChair2 _instance;
	public static AnimateWheelChair2 Instance
    {
        get
        {
            if (!_instance)
            {
                _instance = (AnimateWheelChair2)GameObject.FindObjectOfType(typeof(AnimateWheelChair2));

                if (!_instance)
                {
                    Debug.LogError("AnimateWheelChair2 instance could not be found, make sure to add a bed to the scene, and add the AnimateWheelChair2 script to it");
                } 
            }
