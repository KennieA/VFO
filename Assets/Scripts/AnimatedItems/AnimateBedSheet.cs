...
public class AnimateBedSheet : MonoBehaviour {
	...
	public class CAnimate
	{
		...
		{
			s = AnimateBedSheet.Instance.GetComponent<Animation>()[animName];
		...
		public void StartAnim()
		{
			if(additive && !s.enabled) {
				...
				AnimateBedSheet.Instance.GetComponent<Animation>().CrossFade(s.name, animFadeTime);
			}
		}
	...
	private static AnimateBedSheet _instance;
	public static AnimateBedSheet Instance
    {
        get
        {
            if (!_instance)
            {
                _instance = (AnimateBedSheet)GameObject.FindObjectOfType(typeof(AnimateBedSheet));

                if (!_instance)
                {
                    Debug.LogError("AnimateBedSheet instance could not be found, make sure to add a bed to the scene, and add the AnimateBedSheet script to it");
                } 
            }
