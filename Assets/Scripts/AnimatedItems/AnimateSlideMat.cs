...
public class AnimateSlideMat : MonoBehaviour {
	...
	public class CAnimate
	{
		...
		{
			s = AnimateSlideMat.Instance.GetComponent<Animation>()[animName];
		...
		public void StartAnim()
		{
			if(additive && !s.enabled) {
				...
				AnimateSlideMat.Instance.GetComponent<Animation>().CrossFade(s.name, animFadeTime);
			}
		}
	...
	private static AnimateSlideMat _instance;
	public static AnimateSlideMat Instance
    {
        get
        {
            if (!_instance)
            {
                _instance = (AnimateSlideMat)GameObject.FindObjectOfType(typeof(AnimateSlideMat));

                if (!_instance)
                {
                    Debug.LogError("AnimateSlideMat instance could not be found, make sure to add a bed to the scene, and add the AnimateSlideMat script to it");
                } 
            }
