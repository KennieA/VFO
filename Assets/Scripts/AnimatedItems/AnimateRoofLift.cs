...
public class AnimateRoofLift : MonoBehaviour
{
    // callback from the remote
    public void MoveUp()
    {
        GameObject go = GameObject.Find(States.Instance.GetStateValue("actionCallbackGameObjectName"));
        if (go) go.SendMessage("SimCallback", "RoofLiftUp");
    }

    public void MoveDown()
    {
        GameObject go = GameObject.Find(States.Instance.GetStateValue("actionCallbackGameObjectName"));
        if (go) go.SendMessage("SimCallback", "RoofLiftDown");
    }
    ...
    public class CAnimate
    {
        ...
        {
            s = AnimateRoofLift.Instance.GetComponent<Animation>()[animName];
        ...
        public void StartAnim()
        {
            if (additive)
            {
                AnimateRoofLift.Instance.GetComponent<Animation>().Blend(s.name, additiveWeight, animFadeTime);
            }
            else
            {
                AnimateRoofLift.Instance.GetComponent<Animation>().CrossFade(s.name, animFadeTime);
            }
        }
    }

    private static AnimateRoofLift _instance;
    public static AnimateRoofLift Instance
    {
        get
        {
            if (!_instance)
            {
                _instance = (AnimateRoofLift)GameObject.FindObjectOfType(typeof(AnimateRoofLift));

                if (!_instance)
                {
                    Debug.LogError("AnimateRoofLift instance could not be found, make sure to add a bed to the scene, and add the AnimateRoofLift script to it");
                }
            }
