...
public class AnimateBorger : MonoBehaviour {
	...
	public class CAnimate
	{
		...
		{
			s = AnimateBorger.Instance.GetComponent<Animation>()[animName];
		...
		{
			if(additive) {
                AnimateBorger.Instance.GetComponent<Animation>().Blend(s.name, additiveWeight, animFadeTime);
			}
			else {
				AnimateBorger.Instance.GetComponent<Animation>().CrossFade(s.name, animFadeTime);
			}
		}
	}
	
	private static AnimateBorger _instance;
	public static AnimateBorger Instance
    {
        get
        {
            if (!_instance)
            {
                _instance = (AnimateBorger)GameObject.FindObjectOfType(typeof(AnimateBorger));

                if (!_instance)
                {
                    Debug.LogError("Borger instance could not be found, make sure to add a bed to the scene, and add the AnimateBorger script to it");
                } 
            }
