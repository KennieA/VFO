...
public class AnimateSling : MonoBehaviour {
	...
	public class CAnimate
	{
		...
		{
			s = AnimateSling.Instance.GetComponent<Animation>()[animName];
		...
		public void StartAnim()
		{
			if(additive && !s.enabled) {
				...
				AnimateSling.Instance.GetComponent<Animation>().CrossFade(s.name, animFadeTime);
			}
		}
	...
	private static AnimateSling _instance;
	public static AnimateSling Instance
    {
        get
        {
            if (!_instance)
            {
                _instance = (AnimateSling)GameObject.FindObjectOfType(typeof(AnimateSling));

                if (!_instance)
                {
                    Debug.LogError("AnimateSling instance could not be found, make sure to add a bed to the scene, and add the AnimateSling script to it");
                } 
            }
