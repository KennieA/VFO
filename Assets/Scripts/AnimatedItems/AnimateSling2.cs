...
public class AnimateSling2 : MonoBehaviour
{
    public class CAnimate
    {
        ...
        {
            s = AnimateSling2.Instance.GetComponent<Animation>()[animName];
        ...
        public void StartAnim()
        {
            ...
            {
                AnimateSling2.Instance.GetComponent<Animation>().CrossFade(s.name, animFadeTime);
            }
        }
    ...
    private static AnimateSling2 _instance;
    public static AnimateSling2 Instance
    {
        get
        {
            if (!_instance)
            {
                _instance = (AnimateSling2)GameObject.FindObjectOfType(typeof(AnimateSling2));

                if (!_instance)
                {
                    Debug.LogError("AnimateSling2 instance could not be found, make sure to add a bed to the scene, and add the AnimateSling2 script to it");
                }
            }
