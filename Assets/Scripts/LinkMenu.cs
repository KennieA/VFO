...
public class LinkMenu : MonoBehaviour
{
            ...
            foreach (var link in links)
            {
                if (GUI.Button(link.Rect, link.text, link.enabled ? enabledStyle : disabledStyle))
                {
                    if (link.enabled)
                    {
                        try
                        {
                            SceneLoader.Instance.SendMessage("Scene" + link.sceneId);
                        }
    ...
    void OnGUI()
    {
        ...
        GUI.Box(backRect, "", GUI.skin.textArea);
	...
	void Start () {
        backRect = LinkPanel.visibleAreaRect;
        float inc = 5;
        backRect.x -= inc;
        backRect.y -= inc;
        backRect.width += inc*2;
        backRect.height += inc*2;
    }
